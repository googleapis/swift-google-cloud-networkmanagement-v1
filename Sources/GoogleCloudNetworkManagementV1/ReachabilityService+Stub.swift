...
extension Clients {
  ...
  class ReachabilityServiceTransport: ReachabilityServiceStub {
    ...
    public func listConnectivityTests(
      ...
      let path = try { () throws -> Swift.String in
        ...
        return "/v1/\(pathVariable0)/connectivityTests"
      }()
      var query = [
      ...
      return try GoogleCloudWkt._ProtoJSONDecoder().decode(
        GoogleCloudNetworkManagementV1.ListConnectivityTestsResponse.self, from: data)
    }
    ...
    public func getConnectivityTest(
      ...
      let (data, _) = try await self.inner.rpc(for: req).get()
      return try GoogleCloudWkt._ProtoJSONDecoder().decode(
        GoogleCloudNetworkManagementV1.ConnectivityTest.self, from: data)
    }
    ...
    public func updateConnectivityTest(
      ...
      let path = try { () throws -> Swift.String in
        guard let pathVariable0 = request.resource.map({ $0.name }), !pathVariable0.isEmpty else {
          throw GoogleCloudGax.RequestError.binding(
            "'request.resource.name' is not set or is empty")
        }
        return "/v1/\(pathVariable0)"
      }()
      var query = [
